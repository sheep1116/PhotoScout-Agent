...
def main(args):
    ...
    with httpx.Client(base_url=args.url + "/v1", timeout=30, trust_env=False) as client:
        ...
        checks = {
            ...
            "candidate_presentation": plan.presentation == "candidates",
            "independent_candidates": len({t.spot_id for t in plan.tasks}) == len(plan.tasks),
            "no_itinerary_routes": not plan.routes,
            "community_isolated": bool(plan.metrics.get("community", {}).get("bilibili")),
        ...
        for name, content in (("candidate-discovery-live.json", report), ("candidate-discovery-plan.json", plan.model_dump(mode="json"))):
