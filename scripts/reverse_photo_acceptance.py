...
def main():
    ...
    with httpx.Client(base_url='http://127.0.0.1:3800/v1',timeout=60,trust_env=False) as client:
        ...
        brief={'destination':'南京市','travel_date':str(today+timedelta(days=1)),'end_date':str(today+timedelta(days=1)),
               'start_local':'00:00','end_local':'23:59','timezone':'Asia/Shanghai','mode':'live','text':'寻找参考照片中的建筑机位与相似构图。',
               'intent':{'categories':['architecture']},'tripod':True,'lenses':[{'name':'24–70mm F4','min_mm':24,'max_mm':70,'max_aperture':4}]}
        started=check(client.post(f"/reference-photos/{photo['id']}/analysis",json={'brief':brief,'mode':'similar'}))
        ...
        brief['reverse_context']={'analysis_id':identifier,'spot_id':analysis['spots'][0]['id'],'days':3}
        ...
        checks={'vision_real':any(e.label=='INFERRED' and e.source_id.startswith('src-reference-vision') for e in plan.evidence),
                'has_candidates':bool(analysis['spots']),'multi_day':len(plan.recreation['windows'])>=2,
                ...
                'uncertainty_visible':plan.recreation['location_confidence']=='low',
        ...
        report={'passed':all(checks.values()),'checks':checks,'photo_id':photo['id'],'analysis_id':identifier,'plan_id':plan_id,
                'image_source':image['source_url'],'visual':analysis['visual'],'candidate_count':len(analysis['spots']),
        ...
        (Path('docs/verification')/'reverse-photo-live.json').write_text(content,encoding='utf-8')
