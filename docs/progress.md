...
完成真实参考图、多源发现层、组合式 PhotographyIntent 与 Place/PhotoSpot/Subject 分离。新增候选机位列表、完整图片查看、作者/授权/EXIF 来源展示及失败降级。实际修复描述性机位名匹配失败、景区子地标匹配与同城越界推荐。

119 项后端测试、8 项浏览器 E2E、62 条离线评测通过。Live 样本 `42fe99cb904b4932acbd1f605b825139` 的 14 项检查通过：荷花栈道到紫峰大厦约 271°，2 个候选、3 个任务、6 张高德参考图均经代理加载。Wikimedia 当前网络超时，Flickr 未配置，不冒称已联网验收。报告与使用指南见 product-upgrade.md。

当前使用 start.ps1 启动的本地进程，原数据库保留；本次没有公网部署、创建外部账户或改动真实 .env。
