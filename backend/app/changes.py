...
async def refresh_proposal(plan, providers):
    ...
    if records and all(e.valid_until > current for e in records):
        return None
    ledger = Ledger()
    conditions = await providers.weather(plan.brief, plan.spots[0].camera, ledger)
    if all(w.label == TruthLabel.UNKNOWN for w in conditions):
    ...
    for task in changed.tasks:
        if task.status == "CANCELLED":
            continue
        weather = min(conditions, key=lambda w: abs((w.at-task.start).total_seconds())).model_copy(deep=True)
        old = task.weather
        ...
        reason = gate(spot, weather, plan.brief, task.start, task.end)
        ...
        score_ledger = Ledger()
        task.score = score(plan.brief, weather, task.start, plan.solar, score_ledger, task.id + suffix)
