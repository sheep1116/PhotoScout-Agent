...
const demo: Brief = {text:'南京紫金山，想拍一组自然、电影感的人像，不想走太远。',destination:'南京紫金山',travel_date:'2026-10-03',start_local:'14:00',end_local:'19:00',timezone:'Asia/Shanghai',intent:{categories:['portrait'],subjects:[],styles:[],light:'golden_hour',preferences:{}},lenses:[{name:'35mm F1.8',min_mm:35,max_mm:35,max_aperture:1.8},{name:'85mm F1.8',min_mm:85,max_mm:85,max_aperture:1.8}],sensor:'full_frame',tripod:false,mode:'mock'};
const initial:Brief = {...demo,text:'',destination:'',travel_date:'',start_local:'',end_local:'',intent:{categories:['landscape'],subjects:[],styles:[],light:'any',preferences:{}},lenses:[],mode:'live'};
...
export default function Home() {
  ...
  const seed = (genre:'portrait'|'cityscape')=>{setError('');destinationEdited.current=true;setLocationNote('已选择演示模板');setBrief({...demo,end_date:demo.travel_date,edited_fields:[],intent:{categories:[genre],subjects:[],styles:[],light:genre==='cityscape'?'blue_hour':'golden_hour',preferences:{}},
    ...
    <main className="main">
      ...
      <div className="page-content">
        ...
        <section ref={resultRef} className="results">
          ...
          tab==='spots'?<><AgentAnswerPanel plan={plan} onSelect={setSelected}/><RecreationResult key={plan.id} plan={plan}/><div className="task-list">{Array.from(new Map(plan.tasks.map(t=>[t.spot_id,t])).values()).map((task,i)=><TaskCard key={task.id} task={task} number={i+1} plan={plan} selected={selected===task.spot_id} onSelect={()=>setSelected(task.spot_id)} cite={cite} change={change} working={working}/>)}</div>{!plan.tasks.length&&<div className="notice">Agent 建议已保留，但暂时没有可生成地图与环境核验卡片的机位；请查看上方建议、来源和核验状态。</div>}</>:
