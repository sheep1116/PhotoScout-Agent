...
import {defaultWindow} from '@/lib/defaults';
import ScoutMap from '@/components/ScoutMap';
import {api, Brief, Evidence, localTime, Plan, Proposal, Task} from '@/lib/types';

const demo: Brief = {text:'南京紫金山，想拍一组自然、电影感的人像，不想走太远。',destination:'南京紫金山',travel_date:'2026-10-03',start_local:'14:00',end_local:'19:00',timezone:'Asia/Shanghai',genre:'portrait',profile:'enthusiast',lenses:[{name:'35mm F1.8',min_mm:35,max_mm:35,max_aperture:1.8},{name:'85mm F1.8',min_mm:85,max_mm:85,max_aperture:1.8}],sensor:'full_frame',tripod:false,max_walk_km:3,accept_tickets:false,crowd_tolerance:'low',mode:'mock'};
const initial:Brief = {...demo,text:'',destination:'',travel_date:'',start_local:'',end_local:'',genre:'landscape',lenses:[],mode:'live'};
...
export default function Home() {
  ...
  const patch = (value: Partial<Brief>)=>{
    if(Object.hasOwn(value,'destination')){destinationEdited.current=true;setLocationNote('使用你填写的目的地。');}
    setBrief(b=>({...b,...value}));
  };
  const chooseProfile = (profile:string)=>patch({profile,...(profile==='phone'?{sensor:'phone',lenses:[],tripod:false}:profile==='family'?{tripod:false}:{})});
  const seed = (genre:'portrait'|'cityscape')=>{setError('');destinationEdited.current=true;setLocationNote('已选择演示模板');setBrief({...demo,end_date:demo.travel_date,genre,intent:{categories:[genre],subjects:[],styles:[],light:genre==='cityscape'?'blue_hour':'golden_hour',mobility:'standard'},
  ...
  const review = async()=>{setError('');setWorking(true);try{const book=await api<{brief:Brief;questions:string[];assumptions:string[]}>('/notebook',{...brief,travel_date:brief.travel_date||null,end_date:brief.end_date||null,start_local:brief.start_local||null,end_local:brief.end_local||null});if(book.questions.length){setError(book.questions.join(' '));return;}setBrief(book.brief);setAssumptions(book.assumptions);setConfirm(true);}catch(e){setError((e as Error).message);}finally{setWorking(false);}};
  ...
  return <div className="shell">
      ...
      <div className="page-content">
        ...
        <div className="workbench">
          <section className="brief-panel">
            ...
            <div className="field-row"><label>目的地<div className="icon-input"><MapPin size={15}/><input aria-label="目的地" value={brief.destination} onChange={e=>patch({destination:e.target.value})}/></div></label><label>拍摄日期<input aria-label="拍摄日期" type="date" value={brief.travel_date} onChange={e=>patch({travel_date:e.target.value,end_date:e.target.value})}/></label></div>
            ...
            <p className="intent-hint" role="status">{locationNote} 定位只填写默认值，不会自动搜索。</p>
            <label>结束日期<input type="date" aria-label="结束日期" value={brief.end_date || brief.travel_date} onChange={e=>patch({end_date:e.target.value})}/></label>
            <p className="intent-hint">时间按 {brief.timezone}；跨午夜可选择次日结束。</p>
    ...
    {confirm&&<div className="overlay" onClick={()=>setConfirm(false)}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="confirm-title" onClick={e=>e.stopPropagation()}><button className="close" aria-label="关闭确认" onClick={()=>setConfirm(false)}><X size={20}/></button><div className="modal-icon"><BookOpen size={24}/></div><div className="eyebrow muted">TRIP NOTEBOOK</div><h2 id="confirm-title">让我们对齐这次出发。</h2><p>你确认的需求，将作为整个计划的依据。</p><dl className="notebook"><div><dt>目的地 / 日期</dt><dd>{brief.destination} · {brief.travel_date}</dd></div><div><dt>时间 / 题材</dt><dd>{brief.start_local}–{brief.end_local} · {(brief.intent?.categories || [brief.genre]).map(c=>categoryLabels[c]||c).join(' / ')}</dd></div><div><dt>器材</dt><dd>{brief.lenses.map(l=>l.name).join('、')||'未填写器材，使用通用建议'} · {brief.tripod?'有三脚架':'手持'}</dd></div><div><dt>数据</dt><dd>{brief.mode==='mock'?'Fixture 离线演示，不代表真实现状':'Live 联网；调用百炼与高德，可能按账户规则计费'}</dd></div></dl><div className="notice">{assumptions.map(a=><p key={a}>{a}</p>)}</div><button className="primary" onClick={generate}>确认需求，开始侦察 <ArrowRight size={17}/></button></section></div>}
...
function TaskCard({task,number,plan,selected,onSelect,cite,change,working}: {task:Task;number:number;plan:Plan;selected:boolean;onSelect:()=>void;cite:(ids:string[])=>void;change:(task:Task,reason:string)=>void;working:boolean}) {
  ...
  return <article className={`task-card ${selected?'selected':''} ${task.status==='CANCELLED'?'cancelled':''}`}>
    <div className="task-number"><Compass size={20}/></div><div className="task-main"><div className="task-topline"><span className="task-time"><Clock3 size={13}/>建议拍摄 {new Intl.DateTimeFormat('zh-CN',{timeZone:plan.brief.timezone,month:'numeric',day:'numeric'}).format(new Date(task.start))} {localTime(task.start,plan.brief.timezone)} — {localTime(task.end,plan.brief.timezone)}</span><span className={`tag ${task.status==='CANCELLED'?'red':'amber'}`}>{task.status==='CANCELLED'?'已取消':'暂定 · 待核验'}</span><button className="task-location" onClick={onSelect}><MapPin size={13}/>在图上查看</button></div><h3>{spot.name}</h3><PhotoGallery spot={spot} planId={plan.id}/><p className="composition">{task.composition}</p><p>推荐光线：{({daylight:"日间",golden_hour:"黄金时刻",blue_hour:"蓝调时刻",night:"夜间"} as Record<string,string>)[task.recommended_light || ""] || "按现场条件判断"}</p><div className="spatial"><span>所属地点：{spot.place?.name || spot.name}</span><span>怎么站：{spot.camera_instruction || "精确站位待现场确认"}（{spot.viewpoint_status==='mapped_viewpoint'?'独立地标已匹配，视线待确认':'区域候选，尚非精确机位'}）</span><span>站位：{spot.name} <b>{spot.camera.precision==='EXACT_VERIFIED'?'用户确认点':'区域待确认'}</b></span><span>主体：{spot.subjects.map(s=>s.name).join('、')}</span><span>方向：{task.target_bearing_deg===null?'待确认':`${task.target_bearing_deg}° · 区域示意`}</span></div>
    ...
    <div className="recommendation-reasons"><b>为什么值得拍</b>{(task.reasons||['来源线索与地图位置已关联，具体条件请查看证据。']).map((reason,i)=><p key={i}>{Object.entries(categoryLabels).reduce((text,[key,value])=>text.replaceAll(key,value),reason)}</p>)}</div>
    <details className="task-details"><summary>出行参考与社区经验<ChevronDown size={14}/></summary><div>{(task.travel_advice||['旧记录未提供出行信息，请另行查询地图。']).map((advice,i)=><p key={i}>{advice}</p>)}{plan.claims.filter(c=>c.subject_id===spot.id && c.kind!=='photo').map(c=><p key={c.id}>{c.statement} <button className="tiny-link" onClick={()=>cite(c.evidence_ids)}>来源依据 ↗</button></p>)}</div></details>
    ...
    <details className="task-details"><summary>风险、备选与来源<ChevronDown size={14}/></summary><div>{task.risks.map((r,i)=><p key={i}>• {r}</p>)}<p><b>备选：</b>{task.alternative}</p><button className="tiny-link" onClick={()=>cite(task.evidence_ids)}>查看机位与推荐证据 ↗</button></div></details>
