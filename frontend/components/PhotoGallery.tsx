...
export default function PhotoGallery({spot,planId}: {spot:Spot;planId:string}) {
  ...
  return <section className="photo-gallery" aria-label={`${spot.name}真实参考图`}>
    <div className={`photo-cover ${full?'full':''}`}><Preview key={current.id} photo={current} planId={planId}/><span>{current.relation==='nearby'?'附近参考 · 非精确站位样片':'地点实景参考'} · {current.provider}</span></div>
    ...
    <div className="photo-select" role="group" aria-label="选择参考图片">{photos.map((p,i)=><button key={p.id} aria-pressed={current.id===p.id} onClick={()=>setIndex(i)}>{i+1} · {p.provider}</button>)}<button aria-expanded={full} onClick={()=>setFull(!full)}>{full?'收起完整图片':'展开完整图片'}</button></div>
