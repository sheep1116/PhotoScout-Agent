  ...
  return <section className="map-panel" aria-label="摄影机位关系地图">
    ...
    <div className="map-canvas">
      <svg viewBox="0 0 820 420" className="offline-map" role="img" aria-label="离线区域示意图，非导航地图">
        ...
        {spots.map((s,i)=>{const [x,y]=point(s.camera.lon,s.camera.lat);const task=plan?.tasks.find(item=>item.spot_id===s.id); return <g key={s.id} onClick={()=>onSelect(s.id)} className="map-spot" role="button" tabIndex={0} aria-label={`选择${s.name}`} onKeyDown={e=>{if(e.key==='Enter')onSelect(s.id);}}>
          {s.subjects.filter(t=>t.position).map((t,j)=>{const [tx,ty]=point(t.position!.lon,t.position!.lat);const direction=task?.subject_bearings_deg?.[t.name];const distance=task?.subject_distances_km?.[t.name];return <g key={j}><line x1={x} y1={y} x2={tx} y2={ty} stroke="#83964b" strokeDasharray="4 5" markerEnd="url(#arrow)"/><rect x={tx-4} y={ty-4} width="8" height="8" fill="#718579"/><text x={tx+12} y={ty} fontSize="11" fill="#6a7861">{t.name}{direction===undefined?'':` · ${direction}°`}{distance===undefined?'':` · ${distance}km`}</text></g>;})}
